...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const { user, role, shopId, loading, signOut } = useAuth();
  ...
  const navItems = [
    ...
    ...(role === 'owner' ? [{ path: '/staff-account', label: 'Staff Account', icon: Shield }] : []),
  ];
        ...
        <aside className="hidden lg:block w-64 min-h-screen bg-white border-r shadow-sm">
          <div className="sticky top-0 p-6">
            ...
            <div className="mb-4 rounded-lg border bg-gray-50 px-3 py-2">
              ...
              <p className="text-xs uppercase tracking-wide text-gray-500 mt-1">{role}</p>
