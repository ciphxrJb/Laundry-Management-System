...
### Phase 1: Auth Context & Data Isolation (Immediate - High Priority)
**Goal:** Fix authentication to resolve real shop/role context and enforce shop boundaries.

#### Tasks:
1. **Fix `api.getMe()`** → Query `memberships` table for user's active shop, organization, and role.
2. **Add Shop Filtering to API Calls** → Pass `shopId` to all data fetches (orders, customers, dashboard).
3. **Enforce Shop Filtering in Backend** → Update repositories to filter queries by `shopId`.
4. **Tighten RLS Policies** → Restrict data access to user's shops only.
5. **Test Isolation** → Verify users can't access other shops' data.
